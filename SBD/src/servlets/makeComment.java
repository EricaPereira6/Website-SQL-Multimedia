	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if ((!type.isEmpty() && type != null) || (type.equals("admin") && user != null && !user.isEmpty()) ||
			...
			try {
				...
				String commenter = request.getParameter("comentadoPor");
				int star = Integer.parseInt(request.getParameter("classificacao"));
				String content = request.getParameter("conteudo");
				
				Boolean success = false;
				
				if (ComentarioDAO.hasUserComment(commenter, title, publisher)) {
					success = ComentarioDAO.mudarClassificacao(commenter, title, publisher, star, content);
				}
				else {
					success = ComentarioDAO.criarComentario(commenter, title, publisher, star, content);
				}
