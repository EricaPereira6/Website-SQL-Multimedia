...
@WebServlet("/getResourceComments")
public class getResourceComments extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public getResourceComments() {
		super();
	}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if ((!type.isEmpty() && type != null) || (type.equals("admin") && user != null && !user.isEmpty()) ||
			...
			try {
				...
				ArrayList<Comentario> cms = ComentarioDAO.getComentariosbyRecurso(title, publisher);
				
				String recursosString = "";
				JSONObject recursos = new JSONObject();
			
				for (Comentario c : cms) {
			
					recursosString += c.toString().substring(0, c.toString().length() - 1);
					recursosString += "|";
